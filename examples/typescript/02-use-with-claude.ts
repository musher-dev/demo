...
const commitSkill = bundle.skill("write-commit-message");
if (!commitSkill) throw new Error("Skill 'write-commit-message' not found");

const commitResponse = await client.messages.create({
	model: "claude-sonnet-4-5",
	max_tokens: 512,
	system: commitSkill.content(),
	messages: [
		{
			role: "user",
			content:
...
const reviewerAgent = bundle.agent("code-reviewer");
if (!reviewerAgent) throw new Error("Agent 'code-reviewer' not found");
